...
function handleFileUpload() {
    ...
    fileInput.addEventListener("change", async () => {
        let extractedTexts = [];  // ✅ Store extracted text from all files

        for (let file of fileInput.files) {
            let fileType = file.name.split('.').pop().toLowerCase();

            if (fileType === "txt") {
                extractedTexts.push(await processTextFile(file));
            } else if (fileType === "docx") {
                extractedTexts.push(await processDocxFile(file));
            } else if (fileType === "pdf") {
                extractedTexts.push(await processPdfFile(file));
            } else {
                alert("❌ Invalid file type. Please upload a .txt, .docx, or .pdf file.");
            }
        }

        if (extractedTexts.length > 0) sendToAPI(extractedTexts);
        fileInput.remove();
    });
}
...
async function sendToAPI(extractedTexts) {
    ...
    try {
        let response = await fetch("https://flask-summarizer-1-khc9.onrender.com/generate-ppt", {
            ...
            body: JSON.stringify({ texts: extractedTexts })  // ✅ Send as an array
        });
