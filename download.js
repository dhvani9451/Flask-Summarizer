...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function sendToAPI(extractedText) {
      ...
      console.log("Extracted Text:", extractedText); // Add this line
      try {
  ...
  async function processDocxFileData(data) {
      try {
          ...
          console.log("Mammoth.js Output:", result.value);

          let sanitizedText = result.value;
          ...
          // More aggressive sanitization: remove anything that isn't alphanumeric, space, or common punctuation
          sanitizedText = sanitizedText.replace(/[^a-zA-Z0-9\s.,?!]/g, '');  // Add this line

          // Trim whitespace
          sanitizedText = sanitizedText.trim();  // Add this line

          // Truncate the text
          const maxLength = 10000;
          sanitizedText = sanitizedText.substring(0, maxLength);

          await sendToAPI(sanitizedText);
      } catch (error) {
