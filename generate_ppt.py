...
def add_slide(prs, title, content):
    """Adds a slide ensuring proper bullet point formatting."""
    ...
    apply_bullet = True  # ✅ Ensures bullet only for new points
    ...
    for paragraph in content:
        ...
        # ✅ Apply bullet only if it's a new point, not every line
        if apply_bullet:
            p.level = 0  # ✅ PowerPoint's default bullet
        else:
            p.level = 1  # ✅ No bullet for continuous text (paragraph)

        apply_bullet = paragraph.endswith(".")  # ✅ Bullet for next new point only
