...
def add_slide(prs, title, content):
    ...
    for i, paragraph in enumerate(content):
        ...
        # Apply bullet point only to the FIRST paragraph on the slide
        if i == 0:
            p.level = 0  # PowerPoint's default bullet
        else:
            p.level = 1  # Indent without bullet (Crucial fix)
            # Create indent without bullet
            p.text_frame = text_frame
            # Ensure there are no unwanted bullet points
            p.bullet = None
