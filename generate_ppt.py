...
# Modify clean_text to split based on line breaks
def clean_text(text):
...
def add_slide(prs, title, content):
    ...
    # Group lines into paragraphs based on new thoughts
    grouped_content = []
    current_paragraph = ""

    for line in content:
        current_paragraph += line.strip() + " "
        if line.strip().endswith("."):  # Treat as a new thought if it ends with a full stop
            grouped_content.append(current_paragraph.strip())
            current_paragraph = ""
    ...
    # Add bullet points for grouped content
    for i, paragraph in enumerate(grouped_content):
        p = text_frame.add_paragraph()
        p.text = paragraph
        p.font.size = Pt(16)  # Slightly smaller font for better fit
        p.space_after = Pt(8)  # Adjust spacing for readability
        p.level = 0  # Apply bullet points only to grouped paragraphs
