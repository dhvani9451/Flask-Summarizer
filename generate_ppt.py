...
def clean_text(text):
    ...
    # Remove unwanted characters but keep sentence terminators and numbers
    text = re.sub(r'[*#]', '', text)
    text = re.sub(r'[^A-Za-z0-9.,;!?()\s\n]', '', text)  # Keep sentence punctuation and numbers
    text = re.sub(r'\s+', ' ', text).strip()  # Remove extra spaces
    text = re.sub(r'\n\s*\n', '\n\n', text)  # Preserve intentional line breaks

    # ✅ Split paragraphs while keeping full stops and numbers
    paragraphs = [p.strip() for p in text.split("\n\n") if p.strip()]
    
    # ✅ Further split paragraphs at full stops or numbered points
    structured_text = []
    for para in paragraphs:
        # Split at sentence boundaries but preserve punctuation and numbers
        sentences = re.split(r'(?<=\d\.)\s+|(?<=[.!?])\s+', para)  # Detects "1. " or "Sentence."
...
def add_slide(prs, title, content):
    ...
    for paragraph in content:
        ...
        # ✅ Add bullet only if the sentence ends with a full stop or starts with a number
        if re.match(r'^\d+\.', paragraph) or paragraph.endswith('.'):
            p.level = 0  # Apply bullet point
        else:
    ...
    for filename, text in file_texts.items():
        ...
        for paragraph in structured_text:
            ...
            for i, wrapped_line in enumerate(wrapped_lines):
                # ✅ Apply bullet only for sentences ending with '.' or numbered points
                if re.match(r'^\d+\.', wrapped_line) or wrapped_line.endswith('.'):
                    current_slide_text.append(wrapped_line)
                else:
                    # Append continuation text to last point
