...
def add_slide(prs, title, content):
    """Adds a slide with properly formatted bullet points and paragraphs."""
    ...
    for paragraph in content:
        ...
        # ✅ Apply bullet points only to key points (short sentences)
        if len(paragraph.split()) <= 20 and "." not in paragraph:
            p.level = 0  # First-level bullet point
        else:
            p.level = 1  # Paragraph formatting (without bullet)
