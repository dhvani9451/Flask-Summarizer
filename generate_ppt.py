...
# ✅ Function to add slides with text, bullet points, tables, and images
def add_slide(prs, title, content):
    ...
    # ✅ Detect if content is an image path
    if isinstance(content, str) and (content.lower().endswith(('.png', '.jpg', '.jpeg'))):
        if os.path.exists(content):  # Check if image file exists
            slide.shapes.add_picture(content, Inches(1), Inches(1.5), Inches(8), Inches(5))
        return

    # ✅ Detect if content is a table (list of lists)
    ...
    for i, paragraph in enumerate(content):
        ...
        if i > 0:
            p.space_before = Pt(14)  # Space between points & paragraphs

        # ✅ Use bullet points only for short lines (not full paragraphs)
...
def add_table(slide, table_data):
    ...
    table_shape = slide.shapes.add_table(rows, cols, Inches(1), Inches(1.5), Inches(8.5), Inches(5)).table

    # ✅ Formatting table content
    for r, row in enumerate(table_data):
        for c, cell_text in enumerate(row):
            table_shape.cell(r, c).text = str(cell_text)

# ✅ Function to create a structured PowerPoint presentation
def create_presentation(title, text_data):
    ...
    for line in text_data:
        if isinstance(line, list):  # ✅ Detect table data
            add_slide(prs, "Table Data", line)
        elif isinstance(line, str) and line.lower().endswith(('.png', '.jpg', '.jpeg')):  # ✅ Detect images
            add_slide(prs, "Image Slide", line)
