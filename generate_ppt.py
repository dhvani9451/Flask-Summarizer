...
def format_text_with_bullets(text):
    ...
    for sentence in sentences:
        ...
        for word in words:
            ...
            if len(current_line) == 12:  # If 12 words are reached
                formatted_lines.append(" ".join(current_line))  # Join words into a line
                current_line = []  # Reset for the next line
        
        if current_line:  # Add any remaining words as a line
...
def create_presentation(title, text):
    ...
    max_lines_per_slide = 12  # Maximum lines per slide
    current_slide_text = []

    for line in formatted_text:
        current_slide_text.append(line)

        if len(current_slide_text) >= max_lines_per_slide:  # If line limit is reached
